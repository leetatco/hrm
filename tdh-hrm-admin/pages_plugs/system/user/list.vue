<template>
	<view class="page-body">
		...
		<view class="vk-table-button-box">
			...
			<el-button type="warning" size="small" icon="el-icon-unlock" :disabled="!table1.selectItem"
				@click="resetWeixinBtn">解绑微信</el-button>
		</view>
		<!-- 自定义按钮区域结束 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 解绑微信按钮
			async resetWeixinBtn() {			
				let item = that.getCurrentRow(true);	
				
				console.log("item:",item[`wx_openid`]);	
					
				let res = await vk.callFunction({
					url: 'admin/hrm/employees/sys/unBindWeixin',
					title: '请求中...',
					data: {
						item: item,
						provider: 'wx',
