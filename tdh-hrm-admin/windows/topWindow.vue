...
<script>
	...
	export default {
		...
		// 组件挂载完毕时
		async mounted() {
			vk.menuTabs = this.$refs.menuTabs;
		...
		methods: {
			...
			async viewApplication() {
				...
				// await this.startPolling();
				// 跳转到待办审核
				vk.navigateTo(`/pages/bpmn/application-form/list`);
			},
			...
			async loadUnreadCount() {
				try {
					...
					if (res.code === 0) {
						...
						vk.setVuex('$user.unReadCount', this.unreadCount)
					}
				} catch (error) {
			...
			async loadUnProcessCount() {
				try {
					...
					if (res.code === 0) {
						this.unProcessCount = res.total || 0;
						vk.setVuex('$user.unProcessCount', this.unProcessCount)
					}
				} catch (error) {
					console.error('加载未读通知数量失败:', error);
				}
			...
			async startPolling() {
				// 每30秒检查一次新通知	
				console.log("第一次执行")
				await Promise.all([this.loadUnreadCount(), this.loadUnProcessCount()]);
				// 每30秒检查一次新通知
