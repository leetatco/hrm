<template>
	<view class="page-body">
		<!-- 表格搜索组件 -->
		<vk-data-table-query v-model="queryForm.formData" :columns="queryForm.columns" @search="search">
			<template v-slot:_add_time>
				<vk-data-input-date-time v-model="queryForm.formData._add_time"
		...
		<vk-data-table v-else ref="table1" :action="table1.action" :columns="table1.columns"
			:query-form-param="queryForm" :custom-right-btns="table1.customRightBtns" :selection="true" :row-no="true"
	...
	export default {
				...
				table1: {
					...
					customRightBtns: [{
						...
						},
						{
							...
							type: 'primary',
							show: (item) => {
								return this.canHandleTask(item) && item.node_type !== 'return' && item.status ==
									'pending';
							},
							onClick: (item) => {
						...
						},
						{
							...
							show: (item) => {
								return item.node_type === 'return' && this.canHandleReturnTask(item) && item
									.status == 'pending';
							},
							onClick: (item) => {
		...
		methods: {
			...
			async loadFormTypes() {
				try {
					...
					if (res.code === 0 && res.rows) {
						...
						// 更新查询条件中的申请类型选项
						const formTypeColumn = this.queryForm.columns.find(c => c.key === 'form_type_code');
						if (formTypeColumn) {
			...
			canWithdrawApplication(application) {
				...
				const userInfo = vk.getVuex('$user.userInfo');
				return application.applicant_id === userInfo.username && application.status === 'returned'
