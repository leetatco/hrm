	...
	export default {
		...
		data() {
			return {
				...
				table1: {
					...
					columns: [{
							key: "form_data.business_trip_title",
							title: "出差标题",
							type: "text",
							width: 200,
						...
						{
							key: "form_data.business_trip_type",
							title: "出差类型",
							type: "text",
							width: 100,
							formatter: (value) => {
								return value === 'domestic' ? '国内出差' : (value === 'international' ? '国外出差' :
									value);
							}
						},
						{
		...
		methods: {
			...
			syncQueryOptions() {
				...
				const tripTypeField = this.formSchema.fields.find(f => f.name === 'business_trip_type');
				if (tripTypeField && tripTypeField.options) {
					const tripTypeColumn = this.queryForm1.columns.find(c => c.key === 'form_data.business_trip_type');
					if (tripTypeColumn) {
						tripTypeColumn.data = tripTypeField.options;
					}
				}
			},
			...
			async handleFormSubmit(formData) {
				...
				try {
					...
					const title = formData.form_data.business_trip_title || `${userInfo.username}的出差申请`;
					const submitData = {
						...formData,
			...
			getTaskNameFromHistory(history) {
				if (history.task_data && history.task_data.node_info) return history.task_data.node_info.node_name;
