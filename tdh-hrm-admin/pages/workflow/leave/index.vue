	...
	export default {
		...
		data() {
			return {
				...
				formSchema: null,
				formTypeCode: 'LEAVE_APPLICATION', // 请假申请表类型代码
				// 表单类型配置缓存
				formTypeConfigs: {},
				...
				table1: {
					...
					columns: [{
						...
						{
							key: "form_data.leave_type",
							title: "请假类型",
							type: "text",
							width: 100,
							formatter: (value) => {
								return this.getFieldOptionLabel('leave_type', value) || value;
							}
						},
						{
							key: "form_data.items",
		...
		methods: {
			...
			syncQueryOptions() {
				...
				if (leaveTypeField && leaveTypeField.options) {
					const leaveTypeColumn = this.queryForm1.columns.find(c => c.key === 'form_data.leave_type');
					if (leaveTypeColumn) {
						leaveTypeColumn.data = leaveTypeField.options;
					}
				}
			},
			...
			async handleFormSubmit(formData) {
				...
				try {
					...
					const title = formData.form_data.leave_title || `${userInfo.username}的请假申请`;
