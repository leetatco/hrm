<template>
	<view class="page-body">
		...
		<dynamic-form-dialog v-model="formDialog.show" :title="formDialog.title" :form-schema="formSchema"
			:form-type-code="formTypeCode" :initial-data.sync="formDialog.data" :butVisible="true"
	...
	export default {
			...
			return {
				...
				table1: {
					...
					columns: [{
						...
						{
							key: "form_data.overtime_type",
							title: "加班类型",
							type: "text",
							width: 100,
							formatter: (value) => {
								return this.getFieldOptionLabel('overtime_type', value) || value;
							}
						},
						{
		...
		methods: {
			...
			syncQueryOptions() {
				...
				const overtimeTypeField = this.formSchema.fields.find(f => f.name === 'overtime_type');
				if (overtimeTypeField && overtimeTypeField.options) {
					const overtimeTypeColumn = this.queryForm1.columns.find(c => c.key === 'form_data.overtime_type');
					if (overtimeTypeColumn) {
						overtimeTypeColumn.data = overtimeTypeField.options;
					}
				}
			},
