	...
	export default {
		...
		methods: {
			...
			async handleProcess(item) {
				try {
					await this.$confirm('确认执行处理操作，将正式更新员工资料？', '提示', {
						type: 'warning'
					});
				} catch {
					return;
				...
				try {
					const employeeList = item.form_data?.items;
					if (!employeeList || employeeList.length === 0) {
						this.$message.warning('该申请无待处理员工');
						return;
					}

					// 并发更新所有员工
					const updatePromises = employeeList.map((emp, index) => {
						return vk.callFunction({
							url: 'admin/hrm/employees/sys/transfer/updateEmployess',
							data: emp,
							title: `正在更新员工 ${index + 1}/${employeeList.length}...`
						}).then(res => {
							if (res.code !== 0) {
								// 抛出错误，中断 Promise.all
								throw new Error(
									`更新员工 ${emp.employee_name || emp.employee_id || index + 1} 失败: ${res.msg || '未知错误'}`
								);
							}
							return res;
						});
					...
					// 全部成功则继续，任一失败则抛出异常进入 catch
					await Promise.all(updatePromises);

					// 所有员工更新成功后，将申请状态置为 completed
					...
					if (updateRes.code !== 0) {
						throw new Error(`更新申请状态失败: ${updateRes.msg || '未知错误'}`);
					}

					this.$message.success('处理成功，员工资料已更新');
					this.refresh(); // 刷新列表，按钮隐藏
				} catch (error) {
			...
			async handleFormSubmit(formData) {
				...
				try {
					...
					// 计算调动人数
					const employeeCount = formData.form_data.items ? formData.form_data.items.length :
						0;
