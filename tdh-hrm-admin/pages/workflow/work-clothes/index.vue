<template>
	<view class="page-body">
		...
		<!-- 表格组件 -->		
		<vk-data-table v-else ref="table1" :action="table1.action" :columns="table1.columns" class="main-table"
		...
		<!-- 动态表单公共组件 -->
		<dynamic-form-dialog ref="dynamic" v-model="formDialog.show" :title="formDialog.title" :form-schema="formSchema"
...
<script>
	...
	export default {
		...
		watch: {
			// 监听明细变化，自动计算总件数
			'formDialog.data.form_data.items': {
				handler(newVal) {
					...
					this.$set(this.formDialog.data.form_data, 'total_quantity', total);
					this.$set(this.formDialog.data, 'total_quantity', total);
				},
				deep: true,
				immediate: true
			}
		...
		methods: {
			...
			async handleFormSubmit(formData) {
				...
				try {
					...
					const title = formData.form_data.clothes_title || `${userInfo.username}的工衣申请`;
					const submitData = {
						...formData,
						userInfo,
						title,
