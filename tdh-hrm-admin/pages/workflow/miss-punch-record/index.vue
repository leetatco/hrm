	...
	export default {
		...
		data() {
			return {
				...
				table1: {
					...
					columns: [{
							key: "form_data.miss_date",
							title: "签卡日期",
							type: "date",
							width: 120,
							formatter: (value) => {
								return value ? this.formatDate(value, 'yyyy-MM-dd') : '-';
							}
						},
						{
							key: "form_data.miss_type",
							title: "签卡类型",
							type: "text",
							width: 120,
							formatter: (value) => {
		...
		methods: {
			...
			initqueryForm1Options() {
				...
				// 如果签卡原因字段有动态选项，可以同步到查询条件中（目前是静态的，不需要额外处理）
			},
			...
			async handleFormSubmit(formData) {
				...
				try {
					...
					const calculatedValues = {
						miss_date_str: formData.form_data.miss_date || '',
						miss_reason_label: this.getFieldOptionLabel('miss_reason', formData.form_data.miss_reason)
					};
					const title = `${userInfo.username}的签卡申请（${formData.form_data.miss_date || '未知日期'}）`;
					const submitData = {
						...formData,
					...
					const res = await await vk.callFunction({
			...
			submitAfterSimulate() {
				...
				this.handleFormSubmit(this.simulateDialog.data.variables);
			},
