<template>
	<view class="page-body">
		...
		<!-- 员工档案编辑弹窗（离职处理） -->
		<vk-data-dialog v-model="employeeEditDialog.show" :title="employeeEditDialog.title" width="900px" mode="form"
			:close-on-click-modal="false">
			<vk-data-form ref="employeeForm" v-model="employeeEditDialog.data" :rules="employeeForm.rules"
				:columns="employeeForm.columns" label-width="130px" :inline="true" :columnsNumber="2" :border="true">
				<!-- 可添加自定义插槽 -->
				<template v-slot:footer>
					<el-button @click="employeeEditDialog.show = false">取消</el-button>
					<el-button type="primary" @click="submitEmployeeEdit"
						:loading="employeeEditDialog.loading">确定</el-button>
				</template>
			</vk-data-form>
		</vk-data-dialog>
	</view>
	...
	export default {
		...
		data() {
			return {
				...
				// 当前正在处理的申请记录
				_currentProcessItem: null,

				// 员工编辑表单的字段配置（简化版，只显示离职相关字段）
				employeeForm: {
					columns: [{
							key: "employee_id",
							title: "员工工号",
				...
				table1: {
					...
					customRightBtns: [{
						...
						},
						{
							...
							icon: 'el-icon-check',
							show: (item) => {
								item.process_status = item.process_status || 'pending';
								return (this.$hasRole('admin') || this.$hasRole('OA-exportExcel')) &&
					...
					columns: [{
							key: "form_data.resignation_title",
							title: "离职标题",
							type: "text",
							width: 200,
						...
						{
							key: "form_data.type_name",
							title: "离职类型",
							type: "text",
							width: 120
						},
						{
							key: "form_data.resignation_date",
							title: "离职日期",
							valueFormat: "yyyy-MM-dd",
							type: "date",
							width: 120
						...
						{
							key: "form_data.handover_person_name",
							title: "交接人",							
							type: "text",
							width: 120
						},
				...
				queryForm1: {
					...
					columns: [{
						...
						{
							key: "form_data.resignation_type",
							title: "离职类型",
							type: "select",
							width: 150,
							mode: "=",
		...
		methods: {
			...
			async handleProcess(item) {
				try {
					await this.$confirm('确认处理此离职申请，将更新员工档案？', '提示', {
						type: 'warning'
					});
				} catch {
					return;
				...
				// 保存当前申请记录
				this._currentProcessItem = item;

				// 获取申请人ID（假设 applicant_id 即为员工工号）
				const applicantId = item.applicant_id;
				if (!applicantId) {
					this.$message.warning('未找到申请人信息');
					return;
				}

				try {
					const employee = item.form_data.employeeInfo;
					const formData = item.form_data || {};

					// 组装编辑数据（将离职申请中的相关字段填入）
					const editData = {
						...employee,
						// 强制状态为离职
						status: 2,
						// 从申请中提取离职日期、原因、类型、交接人
						resign_date: formData.resignation_date || '',
						resign_desc: formData.resignation_reason || formData.resignation_desc || '',
						// 如果申请中存储的是类型ID，直接使用；否则可能需要映射，此处留空让用户选择
						type_id: formData.resignation_type || '',
						handover_person_id: formData.handover_person_id || ''
					};

					// 打开编辑弹窗
					this.employeeEditDialog.data = editData;
					this.employeeEditDialog.show = true;
			...
			submitEmployeeEdit() {
				...
				this.$refs.employeeForm.validate((valid) => {
					if (!valid) return;
					this.saveEmployeeEdit();
				});
			},

			// 保存员工信息并更新申请状态
			async saveEmployeeEdit() {
				this.employeeEditDialog.loading = true;
				try {
					const editData = this.employeeEditDialog.data;

					// 1. 更新员工档案
					const updateRes = await vk.callFunction({
						url: 'admin/hrm/employees/sys/update',
						data: editData
					});
					if (updateRes.code !== 0) {
						throw new Error(updateRes.msg || '更新员工信息失败');
					}

					// 2. 更新申请状态为 completed
					const applyUpdate = await vk.callFunction({
						url: 'admin/bpmn/application-form/sys/update',
						data: {
							_id: this._currentProcessItem._id,
							process_status: 'completed'
						}
					});
					if (applyUpdate.code !== 0) {
						throw new Error(applyUpdate.msg || '更新申请状态失败');
					}

					this.$message.success('处理成功，员工档案已更新');
					this.employeeEditDialog.show = false;
					this.refresh(); // 刷新列表，按钮自动隐藏
				} catch (error) {
					console.error('处理失败:', error);
					this.$message.error(error.message || '处理失败，请重试');
				} finally {
					this.employeeEditDialog.loading = false;
				}
			},
			...
			async handleFormSubmit(formData) {
				...
				try {
					...
					const title = formData.form_data.resignation_title || `${userInfo.username}的离职申请`;
					const submitData = {
						...formData,
						userInfo,
						title: title,
