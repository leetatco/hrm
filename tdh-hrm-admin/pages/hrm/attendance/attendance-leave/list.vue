		...
		<view class="btn-group">
			<el-row>
				<el-button type="success" size="small" icon="el-icon-circle-plus-outline"
					v-if="$hasRole('admin') || $hasPermission('attendance-leave-add')" @click="addBtn">添加</el-button>
			</el-row>
		</view>
...
<script>
	...
	export default {
			...
			return {
					...
					rightBtns: [
						{
							mode: 'detail_auto',
							title: '详细',
							show: () => this.$hasRole('admin') || this.$hasPermission('attendance-leave-view')
						},
						{
							mode: 'update',
							title: '编辑',
							show: () => this.$hasRole('admin') || this.$hasPermission('attendance-leave-edit')
						},
						{
							mode: 'delete',
							title: '删除',
							show: () => this.$hasRole('admin') || this.$hasPermission('attendance-leave-delete')
						}
					],
					columns: [
						{
				...
				form1: {
					...
					props: {
						...
						rules: {
							leave_code: [{ required: true, message: "假期代码不能为空", trigger: "blur" }],
							leave_name: [{ required: true, message: "假期名称不能为空", trigger: "blur" }],
		...
		methods: {
			...
			addBtn() {
				this.resetForm();
				this.form1.props.action = 'admin/hrm/attendance/sys/leave/add';
				this.form1.props.formType = 'add';
				this.form1.props.title = '添加假期类型';
				this.form1.props.show = true;
			},
			updateBtn({ item }) {
				this.form1.props.action = 'admin/hrm/attendance/sys/leave/update';
				this.form1.props.formType = 'update';
				this.form1.props.title = '编辑假期类型';
				this.form1.props.show = true;
				this.form1.data = { ...item };
			},
			deleteBtn({ item, deleteFn }) {
				deleteFn({
					action: "admin/hrm/attendance/sys/leave/delete",
					data: { _id: item._id }
				});
			}
		}
