	...
	export default {
		data() {
			return {
				...
				table1: {
					// 数据请求地址（需后端实现对应的云函数）
					action: "admin/crm/shareholders/sys/getList",
					// 按钮显示
					rightBtns: [{
							mode: 'detail_auto',
							title: '详细',
		...
		methods: {
			...
			addBtn() {
				this.resetForm();
				this.form1.props.action = 'admin/crm/shareholders/sys/add';
				this.form1.props.formType = 'add';
				this.form1.props.title = '添加股东信息';
				this.form1.props.show = true;
			},
			updateBtn({
				item
			}) {
				this.form1.props.action = 'admin/crm/shareholders/sys/update';
				this.form1.props.formType = 'update';
				this.form1.props.title = '编辑股东信息';
				this.form1.props.show = true;
			...
			deleteBtn({
				...
				deleteFn({
					action: "admin/crm/shareholders/sys/delete",
					data: {
						_id: item._id
					},
				});
