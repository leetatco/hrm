...
class CallFunctionUtil {
  constructor() {
    // 注意：此处的配置会被项目根目录的 app.config.js 覆盖，无需修改此处的配置
    this.config = {
    ...
    this.uploadFile = (obj = {}) => {
      ...
      if (!obj.filePath && obj.file && obj.file.url) obj.filePath = obj.file.url;
      ...
      let promiseRes = new Promise((resolve, reject) => {
        ...
        cloudStorage[provider]
          ...
          .finally(() => {
            ...
            let logMode = config.logger.mode || 0; // 0 正常 1 简洁（折叠）
            // #ifndef WEB || MP
            logMode = 0;
            // #endif
            // #ifdef WEB || MP
            if (logMode === 1) {
              let requestStatus = '成功'; // 0 请求成功 1 请求失败情况1 2 请求失败情况2
              if (Logger.error) {
                requestStatus = '异常';
                colorStr = '#fa3534';
              }
            ...
            // #endif
            if (logMode === 0) console.log(`%c--------【开始】【${providerName}-文件上传】--------`, 'color: ' + colorStr + ';font-size: 12px;font-weight: bold;');
  ...
  callFunctionFail(obj) {
    ...
    if (res.code >= 90001 && errMsg.indexOf('数据库') > -1) {
      ...
      vk.pubfn.debounce(
        ...
        2000,
        false,
        'cloudfunction-network-unauthorized'
      );
      // #endif
  ...
  callFunctionComplete(obj) {
    ...
    if (debugLog) {
      ...
      // #ifdef WEB || MP
      if (logMode === 1 && typeof console.groupEnd === 'function') console.groupEnd();
      // #endif
      Logger = null; // 释放内存
    }
