			...
			<view class="form-card">
				<u-form :model="password" :rules="rules" ref="form" label-position="top">
					...
					<u-form-item label="原密码" prop="oldPassword" border-bottom>
						<u-input v-model="password.oldPassword" :type="showOldPassword ? 'text' : 'password'"
							placeholder="请输入原密码" :clearable="false" :border="false" class="custom-input">
							<template slot="prefix">
								<u-icon name="lock" size="36" color="#2979ff"></u-icon>
							</template>
							<template slot="suffix">
								<u-icon :name="showOldPassword ? 'eye-fill' : 'eye'" size="36" color="#909399"
									@click="showOldPassword = !showOldPassword"></u-icon>
							</template>
						</u-input>
					</u-form-item>
					...
					<u-form-item label="新密码" prop="newPassword" border-bottom>
						<u-input v-model="password.newPassword" :type="showNewPassword ? 'text' : 'password'"
							placeholder="请输入新密码" :clearable="false" :border="false" class="custom-input">
							<template slot="prefix">
								<u-icon name="lock-open" size="36" color="#2979ff"></u-icon>
							</template>
							<template slot="suffix">
								<u-icon :name="showNewPassword ? 'eye-fill' : 'eye'" size="36" color="#909399"
									@click="showNewPassword = !showNewPassword"></u-icon>
							</template>
						</u-input>
					</u-form-item>
					...
					<u-form-item label="确认新密码" prop="passwordConfirmation" border-bottom>
						<u-input v-model="password.passwordConfirmation"
							:type="showConfirmPassword ? 'text' : 'password'" placeholder="请再次输入新密码" :clearable="false"
							:border="false" class="custom-input" @confirm="submitForm">
							<template slot="prefix">
								<u-icon name="checkmark-circle" size="36" color="#2979ff"></u-icon>
							</template>
							<template slot="suffix">
								<u-icon :name="showConfirmPassword ? 'eye-fill' : 'eye'" size="36" color="#909399"
				...
				<view class="password-rules">
					...
					<view class="rule-item">
						<u-icon name="checkmark" size="24" color="#19be6b"
							v-if="password.newPassword.length >= 6"></u-icon>
						<u-icon name="close" size="24" color="#fa3534" v-else></u-icon>
						<text class="rule-text">长度6-20位</text>
					</view>
					<view class="rule-item">
						<u-icon name="checkmark" size="24" color="#19be6b"
							v-if="/[a-zA-Z]/.test(password.newPassword)"></u-icon>
						<u-icon name="close" size="24" color="#fa3534" v-else></u-icon>
						<text class="rule-text">包含字母</text>
					</view>
					<view class="rule-item">
						<u-icon name="checkmark" size="24" color="#19be6b"
							v-if="/\d/.test(password.newPassword)"></u-icon>
						<u-icon name="close" size="24" color="#fa3534" v-else></u-icon>
						<text class="rule-text">包含数字</text>
					</view>
					<view class="rule-item">
						<u-icon name="checkmark" size="24" color="#19be6b"
							v-if="password.newPassword && password.passwordConfirmation && password.newPassword === password.passwordConfirmation"></u-icon>
						<u-icon name="close" size="24" color="#fa3534" v-else></u-icon>
				...
				<view class="submit-section">
					<u-button type="primary" shape="circle" :disabled="!canSubmit" @click="submitForm" :customStyle="{
							height: '88rpx',
							fontSize: '32rpx',
							fontWeight: '500'
						}">
