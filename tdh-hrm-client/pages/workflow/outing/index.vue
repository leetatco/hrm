			...
			<scroll-view scroll-y class="list-scroll" :refresher-enabled="true" :refresher-triggered="refreshing"
				...
				<view class="list-container">
						...
						<view class="item-header">
							<view class="title-wrapper">
								...
								<text class="item-title">
									{{ item.form_data?.outing_title || '未命名外出申请' }}
								</text>
							</view>
						...
						<view class="item-content">
							<view class="info-grid">
								...
								<view class="info-item">
									...
									<text class="info-value">{{ item.form_data?.outing_date || '未指定' }}</text>
								</view>
								<view class="info-item">
									<text class="info-label">时间段</text>
									<text class="info-value">
										{{ item.form_data?.start_time || '' }} - {{ item.form_data?.end_time || '' }}
									</text>
								</view>
								<view class="info-item">
								...
								<view class="info-item full-width">
									...
									<text class="info-value">{{ item.form_data?.outing_reason || '未填写' }}</text>
								</view>
								<view class="info-item">
					...
					<view class="empty-wrapper" v-if="!loading && tableData.length === 0">
						<u-empty mode="data" icon="/static/empty.png" text="暂无外出申请记录">
		...
		<u-popup v-model="detailDialog.show" :mode="popupStyle.mode" :closeable="true"
			...
			<view class="detail-dialog">
				<view class="detail-header">
					<text class="detail-title">外出申请详情</text>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			/* ---------- 权限判断 ---------- */
			canEdit(item) {
				const uid = this.vk.getVuex('$user.userInfo')?.username;
