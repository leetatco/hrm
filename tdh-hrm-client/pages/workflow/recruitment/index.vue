			...
			<scroll-view scroll-y class="list-scroll" :refresher-enabled="true" :refresher-triggered="refreshing"
				...
				<view class="list-container">
						...
						<view class="item-header">
							<view class="title-wrapper">
								...
								<text class="item-title">
									{{ item.form_data?.recruitment_title || '未命名招聘' }}
								</text>
							</view>
						...
						<view class="item-content">
							<view class="info-grid">
								<view class="info-item">
									<text class="info-label">职位信息</text>
									<text class="info-value">
										{{ getPositionSummary(item.form_data?.items) }}
									</text>
								</view>
								<view class="info-item">
									<text class="info-label">期望到岗</text>
									<text class="info-value">
										{{ item.form_data?.expected_start_date || '未指定' }}
									</text>
								</view>
								<view class="info-item">
					...
					<view class="empty-wrapper" v-if="!loading && tableData.length === 0">
						<u-empty mode="data" icon="/static/empty.png" text="暂无招聘申请记录">
		...
		<u-popup v-model="detailDialog.show" :mode="popupStyle.mode" :closeable="true"
			...
			<view class="detail-dialog">
				<view class="detail-header">
					<text class="detail-title">招聘申请详情</text>
				</view>
		...
		methods: {
			...
			async handleFormSubmit(formData) {
				...
				try {
					...
					const title = formData.form_data?.recruitment_title || `${userInfo?.username || '用户'}的招聘申请`;
