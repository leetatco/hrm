	...
	<view class="container">
			...
			<scroll-view scroll-y class="list-scroll" :refresher-enabled="true" :refresher-triggered="refreshing"
				...
				<view class="list-container">
					<view class="list-item" v-for="(item, index) in tableData" :key="index">
						<view class="item-header">
							<view class="title-wrapper">
								...
								<text class="item-title">{{ item.form_data?.reimbursement_title || '未命名报销' }}</text>
							</view>
						...
						<view class="item-content">
							<view class="info-grid">
								<view class="info-item">
									...
									<text
										class="info-value">{{ getReimbursementTypeText(item.form_data?.reimbursement_type) }}</text>
								</view>
								<view class="info-item">
									<text class="info-label">合计金额</text>
									<text class="info-value">¥{{ item.form_data?.total_detail_amount || '0' }}</text>
								</view>
								<view class="info-item">
					...
					<view class="empty-wrapper" v-if="!loading && tableData.length === 0">
						<u-empty mode="data" icon="/static/empty.png" text="暂无报销申请记录">
		...
		<u-popup v-model="detailDialog.show" :mode="popupStyle.mode" :closeable="true"
			...
			<view class="detail-dialog">
				<view class="detail-header"><text class="detail-title">报销申请详情</text></view>
			...
			async handleFormSubmit(formData) {
				...
				try {
					...
					const submitData = {
						...
						userInfo,
						title: formData.form_data?.reimbursement_title || '报销申请',
					};
					if (formData._id) {
