	...
	<view class="container">
			...
			<scroll-view scroll-y class="list-scroll" :refresher-enabled="true" :refresher-triggered="refreshing"
				...
				<view class="list-container">
					<view class="list-item" v-for="(item, index) in tableData" :key="index">
						<view class="item-header">
							<view class="title-wrapper">
								<u-icon name="rmb-circle" size="32" color="#2979ff"></u-icon>
								<text class="item-title">{{ item.form_data?.loan_title || '未命名借款' }}</text>
							</view>
							...
							<view class="info-grid">
								<view class="info-item">
									...
									<text class="info-value">¥
										{{ item.form_data?.loan_amount || 0 }}</text>
								</view>
								<view class="info-item">
									<text class="info-label">预计还款</text>
									<text class="info-value">{{ item.form_data?.expected_repay_date || '未指定' }}</text>
								</view>
								<view class="info-item">
					...
					<view class="empty-wrapper" v-if="!loading && tableData.length === 0">
						<u-empty mode="data" icon="/static/empty.png" text="暂无借款申请记录">
		...
		<u-popup v-model="detailDialog.show" :mode="popupStyle.mode" :closeable="true"
			...
			<view class="detail-dialog">
				<view class="detail-header"><text class="detail-title">借款申请详情</text></view>
			...
			async handleFormSubmit(formData) {
				...
				try {
					...
					const submitData = {
						...formData,
						userInfo,
						title: formData.form_data?.loan_title || '借款申请',
					};
					if (formData._id) {
