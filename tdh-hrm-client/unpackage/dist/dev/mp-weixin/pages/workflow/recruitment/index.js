...
const _sfc_main = {
  ...
  data() {
    return {
      ...
      formSchema: null,
      formTypeCode: "RECRUITMENT_APPLICATION",
      formTypeConfigs: {},
      tableData: [],
      totalCount: 0,
      draftCount: 0,
  ...
  methods: {
    ...
    getPositionSummary(positionList) {
      if (!positionList || !Array.isArray(positionList) || positionList.length === 0) {
        return "暂无职位信息";
      }
      const totalPositions = positionList.length;
      const totalQuantity = positionList.reduce((sum, pos) => sum + (pos.quantity || 0), 0);
      return `${totalPositions}个职位，共${totalQuantity}人`;
    },
    ...
    async init() {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/workflow/recruitment/index.vue:420", "初始化失败:", e);
        this.useDefaultFormSchema();
      } finally {
        this.loading = false;
      }
        ...
        fields: [
          {
            ...
            type: "text",
            required: true,
            placeholder: "请输入招聘标题，如：2024年技术研发岗位招聘",
            maxLength: 100,
            defaultValue: ""
          },
          {
          ...
          },
          {
            ...
            type: "array<object>",
            required: true,
            itemFormat: "${position_name} × ${quantity}人",
            showAdd: true,
            showClear: true,
            showSort: true,
            defaultValue: {
          ...
          },
          {
            ...
            type: "textarea",
            required: false,
            placeholder: "请详细说明招聘背景、团队情况、特殊要求等",
            rows: 4,
            maxLength: 1e3,
            defaultValue: ""
          },
          {
            ...
            maxSize: 10,
            maxCount: 10,
            description: "支持上传岗位说明书、招聘计划等相关文档"
          }
        ],
        layout: {
          type: "grid",
          ...
          groups: [
            {
              title: "招聘基本信息",
              fields: ["recruitment_title", "expected_start_date"]
            },
            {
              title: "招聘职位明细",
              fields: ["items"],
    ...
    async handleFormSave(formData) {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/workflow/recruitment/index.vue:739", "保存失败:", error);
        common_vendor.index.showToast({
          title: "保存失败",
          icon: "none"
        });
    ...
    async handleFormSubmit(formData) {
      ...
      try {
        ...
        let totalQuantity = 0;
        if (((_a = formData.form_data) == null ? void 0 : _a.items) && Array.isArray(formData.form_data.items)) {
          totalPositions = formData.form_data.items.length;
          totalQuantity = formData.form_data.items.reduce((sum, pos) => sum + (pos.quantity || 0), 0);
        }
        const calculatedValues = {
          total_positions: totalPositions,
          total_quantity: totalQuantity,
          expected_start_date: (_b = formData.form_data) == null ? void 0 : _b.expected_start_date
        };
        const title = ((_c = formData.form_data) == null ? void 0 : _c.recruitment_title) || `${(userInfo == null ? void 0 : userInfo.username) || "用户"}的招聘申请`;
        const submitData = {
          ...formData,
        ...
        common_vendor.index.__f__("error", "at pages/workflow/recruitment/index.vue:802", "提交失败:", e);
        common_vendor.index.showToast({
          title: "提交失败",
          icon: "none"
        });
    ...
    async handleSimulate(formData) {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/workflow/recruitment/index.vue:853", "试算失败:", e);
        common_vendor.index.showToast({
          title: "试算失败",
          icon: "none"
        });
    ...
    async loadProcessFlow(item) {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/workflow/recruitment/index.vue:908", "加载审批流程失败:", e);
      }
    },
    async loadStatusHistory(item) {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/workflow/recruitment/index.vue:943", "加载状态历史失败:", e);
      }
    },
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    d: common_vendor.sr("statusFilter", "b8b1eb6f-0"),
    e: common_vendor.p({
      ["active-color"]: "#2979ff"
    }),
    f: common_vendor.p({
    ...
    j: common_vendor.o(($event) => $options.handleStatusChange(""), "28"),
    k: common_vendor.t($data.draftCount),
    l: common_vendor.o(($event) => $options.handleStatusChange("draft"), "03"),
    m: common_vendor.t($data.pendingCount),
    n: common_vendor.o(($event) => $options.handleStatusChange("pending"), "90"),
    ...
    q: common_vendor.f($data.tableData, (item, index, i0) => {
      ...
      return common_vendor.e({
        a: "b8b1eb6f-4-" + i0,
        b: common_vendor.t(((_a = item.form_data) == null ? void 0 : _a.recruitment_title) || "未命名招聘"),
        ...
        f: common_vendor.t($options.getPositionSummary((_b = item.form_data) == null ? void 0 : _b.items)),
        g: common_vendor.t(((_c = item.form_data) == null ? void 0 : _c.expected_start_date) || "未指定"),
    ...
    v: common_vendor.o($options.addBtn, "a0"),
    w: common_vendor.p({
      type: "primary",
      shape: "circle",
    ...
    A: $data.refreshing,
    B: common_vendor.o((...args) => $options.onPullDownRefresh && $options.onPullDownRefresh(...args), "8d"),
    C: common_vendor.o((...args) => $options.loadMore && $options.loadMore(...args), "ed")
  }), {
    ...
    G: common_vendor.o($options.handleFormSave, "8e"),
    H: common_vendor.o($options.handleFormSubmit, "2a"),
    I: common_vendor.o($options.handleSimulate, "66"),
    J: common_vendor.o($options.previewFile, "eb"),
    K: common_vendor.o($options.closeFormDialog, "67"),
    L: common_vendor.o($options.downloadFile, "46"),
    M: common_vendor.p({
    ...
    N: common_vendor.o(($event) => $data.formDialog.show = $event, "02"),
    O: common_vendor.p({
    ...
    Q: common_vendor.o(($event) => $data.simulateDialog.show = $event, "11"),
    R: common_vendor.p({
      mode: $data.popupStyle.mode,
      closeable: true,
      ["border-radius"]: $data.popupStyle.border_radius,
    ...
    U: common_vendor.o($options.previewFile, "34"),
    V: common_vendor.o($options.downloadFile, "c8"),
    W: common_vendor.p({
    ...
    X: common_vendor.o(($event) => $data.detailDialog.show = $event, "f9"),
    Y: common_vendor.p({
      mode: $data.popupStyle.mode,
      closeable: true,
      ["border-radius"]: $data.popupStyle.border_radius,
    ...
    Z: common_vendor.o($options.confirmDelete, "47"),
    aa: common_vendor.o($options.cancelDelete, "c2"),
    ab: common_vendor.o(($event) => $data.deleteDialog.show = $event, "87"),
    ac: common_vendor.p({
    ...
    ad: common_vendor.o($options.filePreviewClose, "23"),
    ae: common_vendor.o($options.downloadFile, "b5"),
    af: common_vendor.p({
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-b8b1eb6f"]]);
wx.createPage(MiniProgramPage);
//# sourceMappingURL=../../../../.sourcemap/mp-weixin/pages/workflow/recruitment/index.js.map
