<template>
	<view class="application-detail">
				...
				<view class="info-row" v-for="item in basicInfoList" :key="item.key">
					...
					<view class="info-value" :class="item.type">
						<template v-if="item.type === 'status'">
							<u-tag :text="item.value" :type="getStatusType(item.value)" size="mini"
								shape="circle"></u-tag>
						</template>
						<template v-else>
							{{ item.value }}
						...
						<view class="return-item" v-if="returnInfo.task">
							...
							<view class="return-item-content">
								...
								<view class="return-task">
									<u-tag :text="returnInfo.task.task_name" type="warning" size="mini"
										shape="circle"></u-tag>
								</view>
							</view>
						</view>
		...
		<view class="detail-section" v-if="formSchema && formSchema.fields">
			<view class="section-header">
				<u-icon name="file-text" size="36" color="#2979ff" style="margin-right: 10rpx;"></u-icon>
				<text class="section-title">{{ formInfoTitle }}</text>
			</view>
			<view class="form-content">
				<!-- 普通字段网格 -->
				<view class="info-grid" v-if="getNormalFields().length > 0">
					<view class="info-row" v-for="field in getNormalFields()" :key="field.name">
						<view class="info-label">{{ field.label }}：</view>
						<view class="info-value">{{ getFieldDisplayValue(field) }}</view>
					</view>
				</view>

				<!-- array<object> 字段（保留原有样式） -->
				<view class="form-field" v-for="field in getArrayObjectFields()" :key="field.name + '_array'">
					...
					<view class="field-value">
						<view class="array-items" v-if="getFieldValue(field.name) && getFieldValue(field.name).length">
							<view v-for="(item, index) in getFieldValue(field.name)" :key="index" class="array-item">
								...
								<view class="array-item-divider" v-if="index < getFieldValue(field.name).length - 1">
								</view>
							</view>
						</view>
						<text v-else>-</text>
							...
							<view v-for="(file, index) in getFieldValue(field.name)" :key="index" class="file-item">
								...
								<view class="file-actions">
									<u-button type="primary" size="mini" @click="handleFileAction('download', file)"
										:custom-style="{ marginLeft: '10rpx', height: '50rpx', fontSize: '24rpx' }">下载</u-button>
									<u-button type="default" size="mini" @click="handleFileAction('preview', file)"
										:custom-style="{ marginLeft: '10rpx', height: '50rpx', fontSize: '24rpx' }">预览</u-button>
								</view>
							</view>
						</view>
						<view class="empty-file" v-else><text>-</text></view>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="detail-section" v-if="showApprovalFlow && processInfo.tasks && processInfo.tasks.length > 0">
          ...
          comment: '创建申请'
        }" :format-date-fn="formatDate" history-title="审批记录" />
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			clearFormValidation() {
				if (this.$refs.handleFormRef) this.$refs.handleFormRef.clearValidate();
			},
			...
			// 获取各类字段
			getNormalFields() {
				return this.formSchema?.fields?.filter(f => !['textarea', 'file', 'array<object>'].includes(f.type)) || [];
			},
			getTextareaFields() {
				return this.formSchema?.fields?.filter(f => f.type === 'textarea') || [];
			},
			getArrayObjectFields() {
				return this.formSchema?.fields?.filter(f => f.type === 'array<object>') || [];
			},
			getFileFields() {
				return this.formSchema?.fields?.filter(f => f.type === 'file') || [];
			},
			...
			getFieldDisplayValue(field) {
				...
				if (selectTypes.includes(field.type)) {
					// 3a. 如果有静态 options 配置，匹配 label
					if (field.options && Array.isArray(field.options)) {
						const option = field.options.find(opt => opt.value === value);
						if (option) return option.label;
					}
					// 3b. 尝试从 form_data 中取 _label 备用（兼容旧数据）
			...
			formatArrayObjectItem(item, formatConfig) {
				if (!formatConfig) return JSON.stringify(item);
				...
				if (Array.isArray(formatConfig)) {
					const parts = formatConfig.map(fc => {
						...
						const displayVal = fc.transform && typeof fc.transform === 'function' ? fc.transform(val) :
							val;
						return `<span style="color:#666;">${label}:</span> <span style="color:#333;font-weight:500;">${displayVal !== undefined && displayVal !== null ? displayVal : ''}</span>`;
					});
