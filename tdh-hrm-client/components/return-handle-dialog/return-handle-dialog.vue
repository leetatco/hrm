			...
			<template v-slot:handle-form>
				<u-form :model="handleForm" ref="handleFormRef" label-width="150">
					<u-form-item label="处理方式" prop="action" :border-bottom="false" :required="true">
						<u-radio-group v-model="handleForm.action" @change="onHandleActionChange">
							<view class="radio-row">
								<u-radio name="resubmit" size="28" active-color="#2979ff">重新提交</u-radio>
							</view>
							<view class="radio-row">
								<u-radio name="withdraw" size="28" active-color="#fa3534">撤回申请</u-radio>
							</view>
						</u-radio-group>
					</u-form-item>
					...
					<u-form-item 
						label="修改说明" 
						prop="modify_comment" 
						v-if="handleForm.action === 'resubmit'" 
						:border-bottom="false" 
						:required="true"
					>
						<u-input 
							v-model="handleForm.modify_comment" 
							type="textarea" 
					...
					<u-form-item 
						...
						v-if="handleForm.action === 'withdraw'" 
						:border-bottom="false" 
						:required="true"
					>
						<u-input 
							v-model="handleForm.withdraw_reason" 
							type="textarea" 
